...
%   Every such timing change is re-certified against the moving obstacles.
    ...
    % ---------------------------------------------------------------------
    % Validate and normalize the timed route
    % ---------------------------------------------------------------------
    ...
    if isscalar(time_s)
        ...
        pointAudit = azel.audit.auditPlannerPath( ...
    ...
    % ---------------------------------------------------------------------
    % Compress collinear motion and search for feasible transition blends
    % ---------------------------------------------------------------------
    ...
    for eventIndex = 1:numel(events)
        ...
        for pairIndex = 1:size(fractionPairs,1)
            ...
            lastAudit = azel.audit.auditAzElTrajectory( ...
    ...
    % ---------------------------------------------------------------------
    % Assemble and certify the complete command
    % ---------------------------------------------------------------------
    segments = assembleSegments(baseSegments,events);
...
% =========================================================================
% Transition events and blend search
% =========================================================================
...
% =========================================================================
% Bezier construction and segment assembly
% =========================================================================
...
function controlPoints_deg = quinticHermiteBezier( ...
        startPoint_deg,endPoint_deg,startVelocity_deg_s, ...
        endVelocity_deg_s,duration_s)
    % Six control points encode a quintic Hermite segment. Positions and
    % velocities match at both ends; endpoint accelerations are set to zero
    % so adjacent straight/wait segments can meet with C2 continuity.
...
function segment = makeOutputSegment( ...
    ...
    % Analytic segment contract: absolute time bounds, a 6-by-2 Bezier
    % control polygon [azimuth,elevation], and a semantic motion kind.
...
% =========================================================================
% Command sampling and analytic evaluation
% =========================================================================
...
% =========================================================================
% Kinematic bounds and limit certification
% =========================================================================
...
% =========================================================================
% Continuity checks, metrics, and result construction
% =========================================================================
...
function trajectory = makePointTrajectory(path,data,options)
    % Trajectory contract: sampled command arrays plus the authoritative
    % analytic segment list and derived rate/acceleration/jerk diagnostics.
    trajectory = struct;
