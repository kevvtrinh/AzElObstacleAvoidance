...
%CREATEGAUNTLETGIF Run all checks and render both planning runs into one GIF.
    ...
    dynamicIndices = unique([1:3:numel(dynamicResult.path.time_s), ...
        numel(dynamicResult.path.time_s)]);
    ...
    showTitleCard(figureHandle,'Spiral-to-center challenge', ...
        ...
        'Pass threshold: 350 deg of angular travel'},'RUN 2 OF 2');
    isFirstFrame = appendFrame( ...
        figureHandle,gifFile,isFirstFrame,1.4);

    spiralIndices = 1:numel(spiralResult.path.time_s);
    for frameNumber = 1:numel(spiralIndices)
        pathIndex = spiralIndices(frameNumber);
        winding_deg = pathWinding(spiralResult.path,pathIndex, ...
    ...
    showTitleCard(figureHandle,'All gauntlet checks passed', ...
        ...
        dynamicResult.diagnostic.waitTime_s), ...
        sprintf('Spiral: %.1f s arrival, %.1f deg angular travel', ...
        spiralResult.diagnostic.arrivalTime_s,report.spiralWinding_deg), ...
...
function drawDynamicFrame(figureHandle,data,result,pathIndex,isFinal)
    ...
    [maskAz_deg,maskEl_deg] = getMaskFrame(data,maskIndex);
    ...
    scatter(ax,result.path.az_deg(1),result.path.el_deg(1),75, ...
        palette.start,'filled','MarkerEdgeColor','w','LineWidth',1);
    scatter(ax,result.path.az_deg(end),result.path.el_deg(end),85, ...
        palette.goal,'filled','MarkerEdgeColor','w','LineWidth',1);
    scatter(ax,result.path.az_deg(pathIndex), ...
        result.path.el_deg(pathIndex),105,palette.current,'filled', ...
        'MarkerEdgeColor','w','LineWidth',1.4);
    text(ax,result.path.az_deg(1)+3,result.path.el_deg(1)-3,'START', ...
        'Color',palette.text,'FontSize',9,'FontWeight','bold');
    text(ax,result.path.az_deg(end)-3,result.path.el_deg(end)+3,'GOAL', ...
    ...
    metrics = {sprintf('Time  %.1f / %.1f s', ...
        ...
        sprintf('Wait  %.1f s total',result.diagnostic.waitTime_s), ...
...
function drawSpiralFrame(figureHandle,scenario,result,pathIndex, ...
    ...
    scatter(ax,result.path.az_deg(pathIndex), ...
        result.path.el_deg(pathIndex),105,palette.current,'filled', ...
        'MarkerEdgeColor','w','LineWidth',1.4);
    text(ax,result.path.az_deg(1)-1,result.path.el_deg(1)+3,'START', ...
        'HorizontalAlignment','right','Color',palette.text, ...
        'FontSize',9,'FontWeight','bold');
...
function winding_deg = pathWinding(path,pathIndex,centerAzEl_deg,minRadius_deg)
    ...
    winding_deg = rad2deg(sum(abs(diff(angles_rad))));
end


function [az_deg,el_deg] = getMaskFrame(data,timeIndex)
