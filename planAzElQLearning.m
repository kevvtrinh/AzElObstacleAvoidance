...
%   worker; only the compact candidate path returns to the client.
    ...
    plannerTic = tic;
    [blocked,grid,maskStats] = buildAzElOccupancy(planningData,options);
...
function [elIndex,azIndex] = snapToGrid( ...
    ...
    if isPeriodic
        [~,azIndex] = min(abs(shortestAzimuthDeltaDeg( ...
    ...
    for timeIndex = 1:numTimes
        ...
        for goalCell = reshape(ids,1,[])
            ...
            if isPeriodicGrid(grid)
                azimuthDifference_deg = shortestAzimuthDeltaDeg( ...
...
function model = makeActionModel(grid,options)
    ...
    for cellIndex = 1:numCells
        ...
        for action = 1:numActions
            ...
            if isPeriodicGrid(grid)
                dAz_deg = abs(shortestAzimuthDeltaDeg( ...
            ...
            if isPeriodicGrid(grid)
                toAz_deg = fromAz_deg+shortestAzimuthDeltaDeg( ...
                    fromAz_deg,toAz_deg);
            end
...
function path = cellsToPath(cellHistory,timeHistory,grid)
    ...
    if isPeriodicGrid(grid)
        path.az_deg = unwrapAzimuthDeg(wrappedAz_deg,wrappedAz_deg(1));
