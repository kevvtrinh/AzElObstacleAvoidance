...
function testPackageBoundariesAndPublicFacadesResolve(testCase)
    publicNames = [ ...
        "planAzElQLearning", "buildAzElOccupancy", ...
        "smoothAzElTrajectory", "auditPlannerPath", ...
        "auditAzElTrajectory", "shortestAzimuthDeltaDeg", ...
        "interpolateAzimuthDeg", "unwrapAzimuthDeg", ...
        "interpolateAzElObstacleFrame", "selectAzElDisplayPath", ...
        "plotAzElQLearningResult", "animateAzElQLearningPlan"];
    packageNames = [ ...
        "azel.planning.planAzElQLearning", ...
        "azel.mapping.buildAzElOccupancy", ...
        "azel.trajectory.smoothAzElTrajectory", ...
        "azel.audit.auditPlannerPath", ...
        "azel.audit.auditAzElTrajectory", ...
        "azel.geometry.shortestAzimuthDeltaDeg", ...
        "azel.geometry.interpolateAzimuthDeg", ...
        "azel.geometry.unwrapAzimuthDeg", ...
        "azel.geometry.interpolateAzElObstacleFrame", ...
        "azel.visualization.selectAzElDisplayPath", ...
        "azel.visualization.plotAzElQLearningResult", ...
        "azel.visualization.animateAzElQLearningPlan"];

    for functionIndex = 1:numel(publicNames)
        publicPath = which(char(publicNames(functionIndex)));
        implementationPath = which(char(packageNames(functionIndex)));
        verifyNotEmpty(testCase,publicPath,publicNames(functionIndex));
        verifyNotEmpty(testCase,implementationPath,packageNames(functionIndex));
        verifyFalse(testCase,contains(publicPath, ...
            [filesep,'+azel',filesep]));
        verifyTrue(testCase,contains(implementationPath, ...
            [filesep,'+azel',filesep]));
    end

    fractions = [0,0.25,0.5,0.75,1];
    verifyEqual(testCase,interpolateAzimuthDeg(179,-179,fractions), ...
        azel.geometry.interpolateAzimuthDeg(179,-179,fractions), ...
        'AbsTol',1e-12);
end
